...
时序图（Sequence Diagram）是 UML 中最常用的交互图，主要用于展示对象之间交互的**时间顺序**。它强调消息是如何在对象之间发送和接收的。

### 1. 核心元素

| 元素 | 图形符号 | 说明 |
| :--- | :--- | :--- |
| **生命线 (Lifeline)** | 虚线 | 表示对象在一段时间内的存在。顶部通常是矩形，包含对象名。 |
| **激活条 (Activation)** | 细长矩形 | 表示对象正在执行某个操作或处于活动状态的时间段。 |
| **消息 (Message)** | 箭头 | 对象之间的通信。 |
| **组合片段 (Fragment)** | 矩形框 | 用于表示循环 (loop)、选择 (alt)、可选 (opt) 等复杂逻辑。 |

### 2. 消息类型

- **同步消息 (Synchronous Message)**：实心三角箭头 `->`。发送者发送消息后，必须等待接收者处理完毕才能继续执行（阻塞）。
- **异步消息 (Asynchronous Message)**：开放箭头 `->>`。发送者发送消息后，不等待接收者处理，直接继续执行（非阻塞）。
- **返回消息 (Return Message)**：虚线箭头 `-->>`。表示从被调用者返回结果给调用者。
- **自关联消息 (Self Message)**：指向自己的箭头。对象调用自身的方法。
...
- **详细设计**：在编码前理清复杂的对象交互逻辑，特别是跨系统的接口调用。
- **API 设计**：展示客户端与服务端、微服务之间的调用时序。
- **排错分析**：通过绘制时序图复现 Bug 产生的交互路径。
...
## 3.6 状态图（State Diagram）：对象生命周期

状态图（State Machine Diagram）用于描述一个特定对象的所有可能状态，以及引起状态转换的事件。它关注的是**单个对象**的生命周期。

### 1. 核心元素

| 元素 | 图形符号 | 说明 |
| :--- | :--- | :--- |
| **状态 (State)** | 圆角矩形 | 对象所处的条件或情况。 |
| **初始状态 (Initial State)** | 实心圆 ● | 对象生命周期的开始。 |
| **结束状态 (Final State)** | 同心圆 ◉ | 对象生命周期的结束。 |
| **转换 (Transition)** | 带箭头的实线 → | 状态之间的切换，通常标记为 `事件 [守卫条件] / 动作`。 |
...
- **对象生命周期建模**：如订单、审批单、游戏角色等具有复杂状态流转的对象。
- **嵌入式系统**：描述硬件设备的控制逻辑。

## 3.7 组件图与部署图：架构视图

这两类图主要用于系统的**实现视图**和**部署视图**。

### 1. 组件图 (Component Diagram)

描述系统中软件组件（如源代码文件、库、可执行文件）及其相互依赖关系。

- **组件 (Component)**：矩形，左侧带有两个小矩形（UML 1.x）或右上角有组件图标（UML 2.x）。
- **接口 (Interface)**：
  - **提供接口 (Provided Interface)**：棒棒糖形状（圆圈+直线），表示组件提供的服务。
  - **请求接口 (Required Interface)**：半圆+直线，表示组件需要的服务。

### 2. 部署图 (Deployment Diagram)

描述系统的硬件拓扑结构以及软件在硬件上的部署情况。

- **节点 (Node)**：立方体，表示硬件设备（如服务器、手机）或执行环境（如 Tomcat、Docker 容器）。
- **制品 (Artifact)**：部署在节点上的软件文件（如 .jar, .war, .exe）。
...
设计模式本质上是**类与类之间关系的特定组合**。掌握了类图中的六种关系，就能轻松读懂设计模式。

- **继承/实现**：定义了模式的骨架（如策略模式的 Strategy 接口，模板方法模式的抽象类）。
- **聚合/组合**：定义了对象的组装方式（如装饰器模式中 Decorator 持有 Component，组合模式中树形结构）。
- **关联/依赖**：定义了对象的交互方式（如观察者模式中 Subject 持有 Observer 列表）。

在后续章节介绍具体设计模式时，我们将大量使用 **类图** 来展示模式结构，用 **时序图** 来展示模式的动态交互。
